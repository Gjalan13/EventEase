            ...
            <div className="h-48 mt-2 gap-2 p-5 bg-gray-100 font-bold rounded-md relative">
              ...
              <div className="flex justify-start place-items-center text-sm md:text-base font-normal">
                ...
                <div className="ml-6 grid grid-cols-2 gap-x-6 gap-y-2">
                  <div className="">
                    Event Name : <br /><span className=" font-extrabold text-primary-900">{ticket.ticketDetails.eventname.toUpperCase()}</span>
                  </div>
                  
                  <div>
                    Date & Time:<br /> <span className="font-extrabold text-primary-900">{ticket.ticketDetails.eventdate.toUpperCase().split("T")[0]}, {ticket.ticketDetails.eventtime}</span>
                  </div>
                  <div>
                    Name: <span className="font-extrabold text-primary-900">{ticket.ticketDetails.name.toUpperCase()}</span>
                  </div>
                  <div>
                    Price: <span className="font-extrabold text-primary-900"> Rs. {ticket.ticketDetails.ticketprice}</span>
                  </div>
                  <div>
                    Email: <span className="font-extrabold text-primary-900">{ticket.ticketDetails.email}</span>
                  </div>
                  <div>
                    Ticket ID:<br /><span className="font-extrabold text-primary-900">{ticket.ticketDetails._id}</span>
                  </div>
                </div>
              </div>
